...
- [~] B-3 MinSimpleBalanceBolt (MSBBolt) mint/transfer/self-transfer - golden byte-match [B-2]. **mint-lock DONE** Hodos `8e55482` (msbbolt.rs mint_lock_args with 16-byte balance; tx0 lock byte-match; embedded MinSimpleBalanceBolt artifact; lifecycle fixture via emitGolden CASES). 15/15 bolt. REMAINING: MSBBolt transfers + melt unlock (reuse min_simple_bolt simple_spend + ancestor algos, generalized for the balance lockArg in scriptCode decomposition - the balance shifts the CTXScriptCode* push indices by +1).
